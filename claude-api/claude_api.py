...
class Client:
    ...
    def send_message(self, prompt, conversation_id, attachment=None, timeout=1200):
    ...
    async def send_message_stream(self, prompt, conversation_id, attachment=None, timeout=1200):
        url = "https://claude.ai/api/append_message"
            ...
            for data_string in data_strings:
                ...
                if "completion" in data:
                    if data["stop_reason"] == "stop_sequence":
                        asyncio.run(q.put({"completion": data["completion"], "stop_reason": "stop_sequence"}))
                    else:
                        asyncio.run(q.put({"completion": data["completion"], "stop_reason": None}))

        def send_request():
            requests.post(url, headers=headers, data=payload, impersonate="chrome110", timeout=timeout,
